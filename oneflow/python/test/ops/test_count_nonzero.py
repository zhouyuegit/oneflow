...
def compare_with_tensorflow(device_type, input_shape, axis, keepdims, rtol=1e-5, atol=1e-5):
    ...
    def CountNonzoreJob(x=flow.FixedTensorDef(input_shape)):
        with flow.device_prior_placement(device_type, "0:0"):
            return flow.math.count_nonzore(x, axis=axis, keepdims=keepdims)
    x = np.random.rand(*input_shape).astype(np.float32)
    # OneFlow
    of_out = CountNonzoreJob(x).get()
    # TensorFlow
    tf_out = tf.math.count_nonzore(x, axis=axis, keepdims=keepdims)
#    print("tf: ")
#    print(tf_out.numpy())
#    print("of: ")
...
def test_count_nonzore(test_case):
