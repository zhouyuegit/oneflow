...
def upsample(inp, filters, size, apply_dropout=False, const_init=True):
  ...
  deconv = tf.keras.layers.Conv2DTranspose(filters, size, strides=2,
                                    padding='same',
...
def Generator(const_init=True):
  ...
  d1 = downsample(inputs, 64, 4, apply_batchnorm=False, const_init=const_init) # (bs, 128, 128, 64)
  ...
  u7 = upsample(d8, 512, 4, apply_dropout=apply_dropout, const_init=const_init) # (bs, 2, 2, 1024)
  u7 = tf.keras.layers.Concatenate()([u7, d7])
  u6 = upsample(u7, 512, 4, apply_dropout=apply_dropout, const_init=const_init) # (bs, 4, 4, 1024)
  u6 = tf.keras.layers.Concatenate()([u6, d6])
  u5 = upsample(u6, 512, 4, apply_dropout=apply_dropout, const_init=const_init) # (bs, 8, 8, 1024)
...
def Discriminator(const_init=True):
  ...
  down1 = downsample(x, 64, 4, apply_batchnorm=False, const_init=const_init) # (bs, 128, 128, 64)
...
def test():
  # download()
  # path_to_zip = "/home/luchenda/.keras/datasets/facades.tar.gz"
  # PATH = os.path.join(os.path.dirname(path_to_zip), 'facades/')

  # inp, re = load(PATH+'train/100.jpg')
  # inp = tf.transpose(tf.expand_dims(inp, 0), perm=[0,3,2,1])
  # re = tf.transpose(tf.expand_dims(re, 0), perm=[0,3,2,1])
  # plt.imsave('test_inp.png', inp.numpy()/255)
  # plt.imsave('test_re.png', re.numpy()/255)
