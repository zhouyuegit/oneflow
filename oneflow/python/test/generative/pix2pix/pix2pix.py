...
class Pix2Pix():
    ...
    def compare_with_tf(self):
        ...
        def CompareJob(input=flow.FixedTensorDef((self.batch_size, 3, 256, 256)),
                    target=flow.FixedTensorDef((self.batch_size, 3, 256, 256))):
            # out = self._downsample(input, 3, 4, name='d1', const_init=True)
            # out = self._upsample(out, 3, 4, name='u1', const_init=True)
        ...
        target = np.load("target.npy").transpose(0,3,1,2).astype(np.float32, order='C')
        of_out = CompareJob(inp, target).get()
        tf_out = np.load("result.npy").transpose(0,3,1,2).astype(np.float32, order='C')

        print((tf_out-of_out).mean())
        assert np.allclose(of_out.ndarray(), tf_out, rtol=1e-2, atol=1e-2)
    
    def _downsample(self, inputs, filters, size, name, reuse=False,
                   apply_batchnorm=True, trainable=True, const_init=True):
        out = layers.conv2d(inputs, filters, size, 
                         const_init=const_init, reuse=reuse,
    ...
    def _upsample(self, inputs, filters, size, name,
                 apply_dropout=False, trainable=True, const_init=True):

        out = layers.deconv2d(inputs, filters, size, const_init=const_init,
                              trainable=trainable, use_bias=False, name=name+'_deconv')
    ...
    def generator(self, inputs, trainable=True, const_init=False):
        if const_init:
            ...
            apply_dropout = True
        # (n, 64, 128, 128)
        d1 = self._downsample(inputs, 64, 4,
            const_init=const_init, trainable=trainable, apply_batchnorm=False, name='g_d1')
        # (n, 128, 64, 64)
        d2 = self._downsample(d1, 128, 4,
            const_init=const_init, trainable=trainable, name='g_d2')
        # (n, 256, 32, 32)
        d3 = self._downsample(d2, 256, 4,
            const_init=const_init, trainable=trainable, name='g_d3')
        # (n, 512, 16, 16)
        d4 = self._downsample(d3, 512, 4,
            const_init=const_init, trainable=trainable, name='g_d4')
        # (n, 512, 8, 8)
        d5 = self._downsample(d4, 512, 4,
            const_init=const_init, trainable=trainable, name='g_d5')
        # (n, 512, 4, 4)
        d6 = self._downsample(d5, 512, 4,
            const_init=const_init, trainable=trainable, name='g_d6')
        # (n, 512, 2, 2)
        d7 = self._downsample(d6, 512, 4,
            const_init=const_init, trainable=trainable, name='g_d7')
        # (n, 512, 1, 1)
        d8 = self._downsample(d7, 512, 4,
            const_init=const_init, trainable=trainable, name='g_d8')
        # (n, 1024, 2, 2)
        u7 = self._upsample(d8, 512, 4, const_init=const_init, 
            trainable=trainable, apply_dropout=apply_dropout, name='g_u7')
        u7 = flow.concat([u7, d7], axis=1)
        # (n, 1024, 4, 4)
        u6 = self._upsample(u7, 512, 4, const_init=const_init,
            trainable=trainable, apply_dropout=apply_dropout, name='g_u6')
        u6 = flow.concat([u6, d6], axis=1)
        # (n, 1024, 8, 8)
        u5 = self._upsample(u6, 512, 4, const_init=const_init,
            trainable=trainable, apply_dropout=apply_dropout, name='g_u5')
        u5 = flow.concat([u5, d5], axis=1)
        # (n, 1024, 16, 16)
        u4 = self._upsample(u5, 512, 4, 
            const_init=const_init, trainable=trainable, name='g_u4')
        u4 = flow.concat([u4, d4], axis=1)
        # (n, 512, 32, 32)
        u3 = self._upsample(u4, 256, 4, 
            const_init=const_init, trainable=trainable, name='g_u3')
        u3 = flow.concat([u3, d3], axis=1)
        # (n, 256, 64, 64)
        u2 = self._upsample(u3, 128, 4, 
            const_init=const_init, trainable=trainable, name='g_u2')
        u2 = flow.concat([u2, d2], axis=1)
        # (n, 128, 128, 128)
        u1 = self._upsample(u2, 64, 4,
            const_init=const_init, trainable=trainable, name='g_u1')
        u1 = flow.concat([u1, d1], axis=1)
        # (n, 3, 256, 256)
        u0 = layers.deconv2d(u1, self.out_channels, 4, name='g_u0_deconv',
                             const_init=const_init, trainable=trainable)
        u0 = flow.keras.activations.tanh(u0)

        return u0
    
    def discriminator(self, inputs, targets, trainable=True, 
                      reuse=False, const_init=False):
        # (n, 6, 256, 256)
        d0 = flow.concat([inputs, targets], axis=1)
        # (n, 64, 128, 128)
        d1 = self._downsample(d0, 64, 4, name='d_d1',
            apply_batchnorm=False, reuse=reuse, const_init=const_init)
        # (n, 64, 64, 64)
        d2 = self._downsample(d1, 128, 4, name='d_d2', 
                              reuse=reuse, const_init=const_init)
        # (n, 256, 32, 32)
        d3 = self._downsample(d2, 256, 4, name='d_d3',
                              reuse=reuse, const_init=const_init)
        # (n, 256, 34, 34)
        pad1 = flow.pad(d3, [[0,0],[0,0],[1,1],[1,1]])
        # (n, 512, 31, 31)
        conv1 = layers.conv2d(pad1, 512, 4, strides=1, padding='valid', name='d_conv1',
        ...
        # (n, 1, 30, 30)
        conv2 = layers.conv2d(pad2, 1, 4, strides=1, padding='valid', name='d_conv2',
    ...
    def _eval_model(self, model, batch_idx, epoch_idx):
        ...
        gout = model(input[ind:ind+1].astype(np.float32, order='C')).get()
    ...
    def test(self):
        ...
        inp = x[batch_idx*self.batch_size:(batch_idx+1)*self.batch_size].astype(np.float32, order='C')
        target = y[batch_idx*self.batch_size:(batch_idx+1)*self.batch_size].astype(np.float32, order='C')
    ...
    def train(self, epochs=1, save=True):
        ...
        @flow.function(func_config)
        def train_generator(input=flow.FixedTensorDef((self.batch_size, 3, 256, 256)),
                            target=flow.FixedTensorDef((self.batch_size, 3, 256, 256)),
            ...
            gan_loss = flow.nn.sigmoid_cross_entropy_with_logits(label1, g_logits,
        ...
        if save:
            ...
            check_point.save('checkpoint/cp_{}'.format(str(datetime.now().strftime("%Y-%m-%d-%H:%M:%S"))))

if __name__ == "__main__":
    os.environ['ENABLE_USER_OP']='True'
    pix2pix = Pix2Pix()
    pix2pix.train(epochs=10)
    # pix2pix.test()
    # pix2pix.load_facades(mode='test')
