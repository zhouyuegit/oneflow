...
class DetectionDataLoadOp final : public Operator {
 public:
  OF_DISALLOW_COPY_AND_MOVE(DetectionDataLoadOp);
  DetectionDataLoadOp() = default;
  ~DetectionDataLoadOp() = default;

  void InitFromOpConf() override {
    CHECK(op_conf().has_detection_data_load_conf());
    const DetectionDataLoadOpConf& conf = op_conf().detection_data_load_conf();
  ...
  const PbMessage& GetCustomizedConf() const override {
    return op_conf().detection_data_load_conf();
  }
  ...
  LogicalBlobId obn2lbi(const std::string& output_bn) const override {
    ...
    ret.set_blob_name(op_conf()
                          .detection_data_load_conf()
                          .blobs(oneflow_cast<int32_t>(output_bn.substr(4)))
                          .name());
    return ret;
  }

  LogicalNode* NewProperLogicalNode() const override { return new DetectionDataLoadLogicalNode; }

  Maybe<void> InferOutBlobDescs(std::function<BlobDesc*(const std::string&)> GetBlobDesc4BnInOp,
                                const ParallelContext* parallel_ctx,
                                const SbpSignature* sbp_signature,
                                std::function<void(OpContext*)> EnrollOpCtx) const override {
    return InferBlobDescsWithBatchSize(GetBlobDesc4BnInOp,
                                       op_conf().detection_data_load_conf().batch_size());
  }

  Maybe<void> InferBlobDescs(std::function<BlobDesc*(const std::string&)> GetBlobDesc4BnInOp,
                             const ParallelContext* parallel_ctx) const override {
    size_t batch_size = op_conf().detection_data_load_conf().batch_size();
 ...
 private:
  ...
  void VirtualGenKernelConf(std::function<const BlobDesc*(const std::string&)> GetBlobDesc4BnInOp,
    ...
    const DetectionDataLoadOpConf& conf = op_conf().detection_data_load_conf();
    DetectionDataLoadKernelConf* this_kernel_conf = kernel_conf->mutable_detection_data_load_conf();
    DataInstanceProto* data_inst = this_kernel_conf->mutable_data_instance();
    for (const auto& blob_conf : conf.blobs()) {
      DataFieldProto* data_field = data_inst->add_data_fields();
      data_field->set_data_case(blob_conf.data_case());
...
REGISTER_CPU_OP(OperatorConf::kDetectionDataLoadConf, DetectionDataLoadOp);
