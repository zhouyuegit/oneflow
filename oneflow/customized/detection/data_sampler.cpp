...
DataSampler::DataSampler(Dataset* dataset)
    : dataset_(dataset), max_count_(0), gen_(dataset->proto().random_seed()) {}

void DataSampler::SubmitContext(DataSamplerContext* ctx) {
  size_t sample_count = dataset()->Size() / ctx->num_replicas_;
  size_t one_more_sample_rank = dataset()->Size() % ctx->num_replicas_;
...
void DataSampler::GenNewEpochIndexSequence(size_t epoch) {
  ...
  std::vector<int64_t> index_seq(dataset()->Size());
  std::iota(index_seq.begin(), index_seq.end(), 0);
  if (dataset()->proto().shuffle()) { std::shuffle(index_seq.begin(), index_seq.end(), gen_); }
