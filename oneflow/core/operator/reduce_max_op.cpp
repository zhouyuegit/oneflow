...
void ReduceMaxOp::InferBlobDescs(std::function<BlobDesc*(const std::string&)> GetBlobDesc4BnInOp,
...
void ReduceMaxOp::VirtualGenKernelConf(
  ...
  if (conf.axis().empty()) {
    ...
    const PbRf<int32_t>& axis_repeated = op_conf().reduce_max_conf().axis();
    std::vector<int64_t> axis_vec = {axis_repeated.begin(), axis_repeated.end()};
