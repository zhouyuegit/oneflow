...
void ProposalOp::InferBlobDescs(std::function<BlobDesc*(const std::string&)> GetBlobDesc4BnInOp,
  ...
  CHECK(anchors_blob_desc->has_dim0_valid_num_field());
  CHECK_EQ(bbox_pred_blob_desc->data_type(), class_prob_blob_desc->data_type());
  CHECK_EQ(bbox_pred_blob_desc->data_type(), anchors_blob_desc->data_type());
  ...
  const int64_t num_anchors_per_cell = class_prob_blob_desc->shape().At(3);
  ...
  CHECK_EQ(num_anchors, anchors_blob_desc->shape().At(0));
  ...
  *roi_probs_blob_desc = *rois_blob_desc;
  ...
  // datatmp: proposal_inds (N, H * W * A) int32_t
