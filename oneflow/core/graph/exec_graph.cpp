...
void ExecNode::InferBlobDescs(const ParallelContext* parallel_ctx) {
  ...
  CHECK_JUST(op_->InferOutBlobDescsIf(GetBlobDesc4BnInOp, parallel_ctx, sbp_signature,
                                   [this](OpContext* op_ctx) { op_ctx_.reset(op_ctx); }));
  CHECK_JUST(op_->InferTmpBlobDescsIf(GetBlobDesc4BnInOp, parallel_ctx, sbp_signature,
                                   [this](OpContext* op_ctx) { op_ctx_.reset(op_ctx); }));
