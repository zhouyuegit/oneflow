...
template<typename T>
__global__ void SetMaskGpu(const int64_t n, const int64_t count, const T* in, const T* max,
                           int32_t* mask) {
  CUDA_1D_KERNEL_LOOP(i, n) { mask[i] = (in[i] == max[i / count]) ? 1 : 0; }
}

template<typename T>
__global__ void SetWithMaskGpu(const int64_t n, const int64_t count, const T* max_diff,
