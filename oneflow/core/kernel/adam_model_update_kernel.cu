...
template<typename T>
__device__ void UpdateModel(int64_t batch_size, T learning_rate, T l1, T l2, T epsilon,
                            T* model_diff, T* model, T* m, T* v) {
  *model_diff = *m / (sqrt(*v) + epsilon);
  T reg_diff = RegularizeDiff(*model_diff, batch_size, l1, l2, *model);
...
__global__ void UpdateModelGpu(int64_t n, int64_t batch_size, T learning_rate, T l1, T l2, T beta1,
  ...
  CUDA_1D_KERNEL_LOOP(i, n) {
    ...
    UpdateModel(batch_size, learning_rate, l1, l2, epsilon, model_diff + i, model + i, m + i,
                v + i);
  }
}
    ...
    if (do_bias_correction) {
      UpdateModelGpu<true, T>
          <<<BlocksNum4ThreadsNum(n), kCudaThreadsNumPerBlock, 0, ctx->cuda_stream()>>>(
              n, batch_size, learning_rate, l1, l2, beta1, beta2, epsilon, beta1_t, beta2_t,
