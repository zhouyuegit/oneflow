...
__device__ typename std::enable_if<do_bias_correction>::type ScaleMoment(const T beta_t,
                                                                         T* moment) {
  *moment /= (1 - beta_t);
}
...
__device__ void UpdateMomentEstimate(T beta, const T* model_diff, const T* beta_t, T* moment) {
  ...
  *moment = beta * (*moment) + (1 - beta) * PowUtil<power>::pow(*model_diff);
  ...
  ScaleMoment<do_bias_correction>(*beta_t, moment);
}

template<typename T>
__device__ void UpdateModel(T learning_rate, T l1, T l2, T epsilon, T* model_diff, T* model, T* m,
                            T* v) {
  T corrected_direction = *m / (sqrt(*v) + epsilon);
  *model = NaiveUpdateModel(learning_rate, corrected_direction, weight_decay, *model);
}

template<bool do_bias_correction, typename T>
__global__ void UpdateModelGpu(int64_t n, T learning_rate, T weight_decay, T beta1, T beta2,
  ...
  CUDA_1D_KERNEL_LOOP(i, n) {
    ...
    UpdateModel(learning_rate, weight_decay, epsilon, model_diff + i, model + i, m + i, v + i);
  }
}
...
class AdamMdUpdateKernelUtil<DeviceType::kGPU, T> final {
 public:
  static void UpdateModel(DeviceCtx* ctx, int64_t n, T learning_rate, T weight_decay, T beta1,
                          T beta2, T epsilon, bool do_bias_correction,
