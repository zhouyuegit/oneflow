...
template<DeviceType device_type, typename T>
void MaskTargetKernel<device_type, T>::ForwardRecordIdInDevicePiece(
    const KernelCtx& ctx, std::function<Blob*(const std::string&)> BnInOp2Blob) const {
  // do nothing here because it will be set in ForwardDataContent
}
...
//     bbox->set_ltrb(std::min(bbox->left(), one_bbox->left()), std::min(bbox->top(),
//     one_bbox->top()),
...
// template<typename T>
// void MaskTargetKernel<T>::Segm2Mask(const PolygonList& segm, const RoiBBox& fg_roi, size_t
// mask_h,
