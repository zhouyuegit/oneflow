...
void SoftmaxReduceMaxStage0Kernel<device_type, T>::ForwardDataContent(
  ...
  SoftmaxReduceMaxStage0KernelUtil<device_type, T>::SetMask(
      ctx.device_ctx, in_blob->shape().elem_cnt(), count, in_blob->dptr<T>(), max_blob->dptr<T>(),
  ...
  NdarrayUtil<device_type, int32_t>::ReduceSum(
      ...
      XpuVarNdarray<int32_t>(Shape(this->kernel_conf().reduce_sum_conf().kept_dims_shape()),
                             max_count_blob->mut_dptr<int32_t>()),
      XpuVarNdarray<const int32_t>(const_cast<const Blob*>(mask_blob), in_blob->shape().NumAxes()),
      XpuVarNdarray<int32_t>(fw_tmp_int_blob, in_blob->shape().NumAxes()));
}
...
void SoftmaxReduceMaxStage0Kernel<device_type, T>::BackwardDataContent(
  ...
  SoftmaxReduceMaxStage0KernelUtil<device_type, T>::SetWithMask(
      ctx.device_ctx, in_diff_blob->shape().elem_cnt(), count, max_diff_blob->dptr<T>(),
      mask_blob->dptr<int32_t>(), max_count_blob->dptr<int32_t>(), in_diff_blob->mut_dptr<T>());
}
...
struct SoftmaxReduceMaxStage0KernelUtil<DeviceType::kCPU, T> {
  static void SetMask(DeviceCtx* ctx, const int64_t n, const int64_t count, const T* in,
    ...
    FOR_RANGE(int64_t, i, 0, n) { mask[i] = (in[i] == max[i / count]) ? 1 : 0; }
  }
...
ADD_DEFAULT_KERNEL_CREATOR(OperatorConf::kSoftmaxReduceMaxStage0Conf, SoftmaxReduceMaxStage0Kernel,
