...
#define DEFINE_OPKT_UTIL_STATIC_SWITCH_FUNC(return_type, func_name)              \
  DEFINE_STATIC_SWITCH_FUNC(return_type, func_name, MAKE_OPKT_UTIL_SWITCH_ENTRY, \
...
#define DEFINE_OPKT_DEV_SWITCHER(return_type, func_name)                      \
  DEFINE_STATIC_SWITCH_FUNC(return_type, func_name, MAKE_OPKT_SWITCH_ENTRY_1, \
                            MAKE_DEVICE_TYPE_CTRV_SEQ(DEVICE_TYPE_SEQ))
#define MAKE_OPKT_SWITCH_ENTRY_1(func_name, device_type) OpKernelTestUtil<device_type>::func_name
...
void BlobCmp(const std::string& blob_name, const Blob* lhs, DeviceType lhs_device_type,
             const Blob* rhs, DeviceType rhs_device_type) {
  ...
  SwitchBlobCmp(SwitchCase(dev_type, lhs->data_type()), blob_name, lhs, lhs_device_type, rhs,
                rhs_device_type);
}
...
void BlobCopy(Blob* dst, const Blob* src) {
  CHECK_EQ(dst->ByteSizeOfDataContentField(), src->ByteSizeOfDataContentField());
  CudaCheck(cudaMemcpy(dst->mut_dptr(), src->dptr(), dst->ByteSizeOfDataContentField(),
...
void OpKernelTestUtil<DeviceType::kCPU>::BuildKernelCtx(KernelCtx* ctx) {
  ctx->device_ctx = new CpuDeviceCtx(nullptr);
}
...
void OpKernelTestUtil<DeviceType::kGPU>::BuildKernelCtx(KernelCtx* ctx) {
  ...
  ctx->device_ctx = new CudaDeviceCtx(nullptr, cuda_handle);
}
...
template<typename T>
Blob* OpKernelTestUtil<device_type>::CreateBlobWithSpecifiedVal(const BlobDesc* blob_desc,
...
void OpKernelTestUtil<DeviceType::kCPU>::CheckInitializeResult(
  ...
  if (initializer_conf.has_constant_conf()) {
    for (int64_t i = 0; i < blob->shape().elem_cnt(); ++i) {
      ASSERT_FLOAT_EQ(blob->dptr<T>()[i], initializer_conf.constant_conf().value());
    }
...
template<>
template<typename T>
Blob* OpKernelTestUtil<DeviceType::kCPU>::CreateBlobWithSpecifiedValPtr(const BlobDesc* blob_desc,
                                                                        T* val, Regst* regst) {
  Blob* ret = CreateBlob(blob_desc, regst);
  CudaCheck(
      cudaMemcpy(ret->mut_dptr(), val, ret->ByteSizeOfDataContentField(), cudaMemcpyHostToHost));
  return ret;
}

template<DeviceType device_type>
template<typename T>
Blob* OpKernelTestUtil<device_type>::CreateBlobWithRandomVal(const BlobDesc* blob_desc,
                                                             Regst* regst) {
...
template<>
template<typename T>
void OpKernelTestUtil<DeviceType::kCPU>::BlobCmp(const std::string& blob_name, const Blob* lhs,
  ...
  if (IsFloatingDataType(lhs->data_type())) {
    ...
    ASSERT_EQ(memcmp(lhs->dptr(), rhs->dptr(), lhs->ByteSizeOfDataContentField()), 0) << blob_name;
  }
}
...
template<typename T>
Blob* OpKernelTestCase::RandomInitBlob(const std::string& name, const BlobDesc* blob_desc) {
  DeviceType dev_type = GetBlobDeviceType(name);
  Blob* blob = OpKTSwitchHelper<T>::SwitchCreateBlobWithRandomVal(SwitchCase(dev_type), blob_desc,
                                                                  bn_in_op2regst_[name]);
  CHECK(bn_in_op2blob_.emplace(name, blob).second);
  return blob;
}

template<typename T>
void OpKernelTestCase::CheckBlob(const std::string& name, const BlobDesc* blob_desc,
...
template<typename T>
void OpKernelTestCase::ForwardCheckBlob(const std::string& name, const BlobDesc* blob_desc,
                                        const std::vector<T>& val, bool need_random_init) {
...
template<typename T>
void OpKernelTestCase::ForwardCheckBlobWithAnother(const std::string& name,
                                                   const BlobDesc* blob_desc,
                                                   const std::string& expected_exist_blob_name,
                                                   bool need_random_init) {
  forward_asserted_blob_names_.push_back(name);
  CheckBlobWithAnother<T>(name, blob_desc, expected_exist_blob_name, need_random_init);
}
...
template<typename T>
void OpKernelTestCase::BackwardCheckBlob(const std::string& name, const BlobDesc* blob_desc,
                                         const std::vector<T>& val, bool need_random_init) {
...
template<typename T>
void OpKernelTestCase::BackwardCheckBlobWithAnother(const std::string& name,
                                                    const BlobDesc* blob_desc,
                                                    const std::string& expected_exist_blob_name,
                                                    bool need_random_init) {
  backward_asserted_blob_names_.push_back(name);
...
std::function<Blob*(const std::string&)> OpKernelTestCase::MakeGetterBnInOp2Blob() {
...
std::function<BlobDesc*(const std::string&)> OpKernelTestCase::MakeGetterBnInOp2BlobDesc() {
...
std::list<std::string> DiffKernelImplTestCase::AllInputBlobNames() const {
  ...
  for (const auto& bn : output_diff_blob_names_) { all_input_blob_names.push_back(bn); }
...
void DiffKernelImplTestCase::SetBlobNames(const std::list<std::string>& input_bn_in_op,
...
void DiffKernelImplTestCase::InitInputBlobs() {
  for (const auto& bn_in_op : AllInputBlobNames()) {
    ...
    const BlobDesc* blob_desc = BlobDesc4BnInOp(bn_in_op);
    Blob* blob =
        SwitchCreateBlob(SwitchCase(default_device_type()), blob_desc, GetBlobRegst(bn_in_op));
    SwitchBlobCopy(SwitchCase(default_device_type(), DeviceType::kCPU), blob, origin_input_blob);
...
void DiffKernelImplTestCase::CheckMultiRunResults(
    const std::string& base_prefix, const std::list<std::string>& other_prefixes) const {
  for (const auto& checkee_prefix : other_prefixes) {
    for (const auto& bn_in_op : AllOutputBlobNamesWithValidBlob()) {
      ...
      BlobCmp(checkee_bn, checkee_blob, checkee_blob_device_type, base_blob, base_blob_device_type);
    }
  }
}
...
#define INSTANTIATE_OPKERNEL_TEST_CASE_METHODS(T, type_proto)                                 \
  template Blob* OpKernelTestCase::InitBlob<T>(const std::string&, const BlobDesc* blob_desc, \
                                               const std::vector<T>& val);                    \
  template Blob* OpKernelTestCase::RandomInitBlob<T>(const std::string&,                      \
                                                     const BlobDesc* blob_desc);              \
  template void OpKernelTestCase::ForwardCheckBlob<T>(                                        \
      const std::string&, const BlobDesc* blob_desc, const std::vector<T>& val);              \
  template void OpKernelTestCase::ForwardCheckBlob<T>(                                        \
      const std::string&, const BlobDesc* blob_desc, const std::vector<T>& val,               \
      bool need_random_init);                                                                 \
