...
void AnchorTargetKernel<device_type, T>::ForwardInstanceShape(
...
template<DeviceType device_type, typename T>
void AnchorTargetKernel<device_type, T>::ForwardDim0ValidNum(
    const KernelCtx& ctx, std::function<Blob*(const std::string&)> BnInOp2Blob) const {
  // const AnchorTargetOpConf& conf = op_conf().anchor_target_conf();
  // const Blob* images_blob = BnInOp2Blob("images");
  // const int32_t image_height = images_blob->shape().At(1);
...
template<DeviceType device_type, typename T>
void AnchorTargetKernel<device_type, T>::ForwardDataContent(
    const KernelCtx& ctx, std::function<Blob*(const std::string&)> BnInOp2Blob) const {
...
//     num_anchors +=
//         BBoxUtil<MutBBox>::GenerateAnchors(im_height, im_width, fm_stride, scales_vec,
//         ratios_vec,
...
ADD_DEFAULT_KERNEL_CREATOR(OperatorConf::kAnchorTargetConf, AnchorTargetKernel,
