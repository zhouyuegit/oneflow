...
void LeakyReluKernel<device_type, T>::BackwardDataContent(
  ...
  LeakyReluKernelUtil<device_type, T>::Backward(
      ctx.device_ctx, out_blob->shape().elem_cnt(), this->op_conf().leaky_relu_conf().alpha(),
      out_blob->dptr<T>(), BnInOp2Blob("out_diff")->dptr<T>(),
...
struct LeakyReluKernelUtil<DeviceType::kCPU, T> {
  ...
  static void Backward(DeviceCtx* ctx, const int32_t n, const float alpha, const T* y, const T* dy,
                       T* dx) {
    for (int64_t i = 0; i != n; ++i) { dx[i] = y[i] > 0 ? dy[i] : dy[i] * alpha; }
  }
};
