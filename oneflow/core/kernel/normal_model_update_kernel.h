...
bool TriggerWarmup(const NormalModelUpdateOpUserConf& conf, float lr, int64_t cur_batch_num);
float GetWarmupLearningRate(const WarmupConf&, float lr, int64_t cur_batch_num);
float GetDecayedLearningRate(const LearningRateDecayConf&, float lr, int64_t cur_batch_num);
