...
void ProposalKernel<device_type, T>::ForwardDataContent(
  ...
  // });
  // Output(BnInOp2Blob);
  const int64_t post_nms_inds_size = 100;
  Blob* rois_blob = BnInOp2Blob("rois");
  Blob* rois_prob_blob = BnInOp2Blob("roi_probs");
  rois_blob->set_dim0_valid_num(0, post_nms_inds_size);
  rois_prob_blob->set_dim0_valid_num(0, post_nms_inds_size);
  FOR_RANGE(size_t, i, 0, post_nms_inds_size) {
    rois_blob->set_record_id_in_device_piece(i, 0);
    rois_prob_blob->set_record_id_in_device_piece(i, 0);
  }
}
...
//                                           post_nms_inds_blob->mut_dptr<int32_t>(im_index),
//                                           false),
//                             proposals_blob->dptr<T>(im_index));
...
template<DeviceType device_type, typename T>
void ProposalKernel<device_type, T>::ForwardDim0ValidNum(
    const KernelCtx& ctx, std::function<Blob*(const std::string&)> BnInOp2Blob) const {
  // do nothing
}

template<DeviceType device_type, typename T>
void ProposalKernel<device_type, T>::ForwardRecordIdInDevicePiece(
    const KernelCtx& ctx, std::function<Blob*(const std::string&)> BnInOp2Blob) const {
  // do nothing
}

ADD_DEFAULT_KERNEL_CREATOR(OperatorConf::kProposalConf, ProposalKernel, FLOATING_DATA_TYPE_SEQ);
