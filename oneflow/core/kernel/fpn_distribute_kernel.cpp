...
void FpnDistributeKernel<device_type, T>::ForwardDataContent(
  ...
  const FpnDistributeOpConf& conf = this->op_conf().fpn_distribute_conf();
  const int32_t level_count = conf.roi_max_level() - conf.roi_min_level() + 1;
  Blob* roi_indices_blob = BnInOp2Blob("roi_indices");
  FOR_RANGE(size_t, i, 0, 400) {
    KernelUtil<device_type, int32_t>::Set(ctx.device_ctx, 10,
                                          roi_indices_blob->mut_dptr<int32_t>() + i);
  }
  roi_indices_blob->set_dim0_valid_num(0, 400);
  FOR_RANGE(int64_t, level, 0, level_count) {
    Blob* roi_blob_i = BnInOp2Blob("rois_" + std::to_string(level));
    roi_blob_i->set_dim0_valid_num(0, 100);
    FOR_RANGE(int64_t, idx, 0, roi_blob_i->shape().At(0)) {
      roi_blob_i->set_record_id_in_device_piece(idx, 0);
    }
  }
}

template<DeviceType device_type, typename T>
void FpnDistributeKernel<device_type, T>::ForwardDim0ValidNum(
    const KernelCtx& ctx, std::function<Blob*(const std::string&)> BnInOp2Blob) const {
  // do nothing here because it will be set in ForwardDataContent
}

template<DeviceType device_type, typename T>
void FpnDistributeKernel<device_type, T>::ForwardRecordIdInDevicePiece(
    const KernelCtx& ctx, std::function<Blob*(const std::string&)> BnInOp2Blob) const {
...
ADD_DEFAULT_KERNEL_CREATOR(OperatorConf::kFpnDistributeConf, FpnDistributeKernel,
