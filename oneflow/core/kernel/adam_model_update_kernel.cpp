...
void AdamMdUpdateKernel<device_type, T>::UpdateModel(
    DeviceCtx* ctx, T learning_rate, T weight_decay, int64_t next_model_vid,
    std::function<Blob*(const std::string&)> BnInOp2Blob) const {
...
class AdamMdUpdateKernelUtil<DeviceType::kCPU, T> final {
 public:
  static void UpdateModel(
      DeviceCtx* ctx, int64_t n, T learning_rate, T weight_decay, T beta1, T beta2, T epsilon,
      bool do_bias_correction, const T* beta1_t, const T* beta2_t, T* model_diff, T* model,
      T* m, T* v) {
    // first-order moment
    ...
    FOR_RANGE(int64_t, i, 0, n) {
      corrected_direction = m[i] / (std::sqrt(v[i]) + epsilon);
      model[i] = NaiveUpdateModel(learning_rate, corrected_direction, weight_decay, model[i]);
    }
  }
};
