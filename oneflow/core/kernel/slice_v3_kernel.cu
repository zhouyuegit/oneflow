...
template<typename T>
__global__ void SliceV3ForwardGpu(const int64_t n, const int64_t* offset, const T* entire,
                                  T* slice) {
...
class SliceV3GpuKernel final : public KernelIf<DeviceType::kGPU> {
 ...
 private:
  void ForwardDataContent(const KernelCtx& ctx,
    ...
    const SliceV3OpConf& conf = this->op_conf().slice_v3_conf();
    ...
    SliceV3ForwardGpu<T><<<BlocksNum4ThreadsNum(num_output), kCudaThreadsNumPerBlock, 0,
