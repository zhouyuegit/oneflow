...
__device__ int64_t GetInOffset(const int64_t out_offset, const K* indices,
                               ...
                               const int64_t lower_bound, const int64_t gather_dim_size) {
...
__global__ void BatchGatherForwardGpu(const int64_t elem_cnt, const T* in, const K* indices,
  ...
  CUDA_1D_KERNEL_LOOP(i, elem_cnt) {
    const int64_t idx =
        GetInOffset<K>(i, indices, indices_num, instance_size, lower_bound, gather_dim_size);
    if (idx != -1) { out[i] = in[idx]; }
  }
}
...
__global__ void BatchGatherBackwardGpu(const int64_t elem_cnt, const T* out_diff, const K* indices,
  ...
  CUDA_1D_KERNEL_LOOP(i, elem_cnt) {
    const int64_t idx =
        GetInOffset<K>(i, indices, indices_num, instance_size, lower_bound, gather_dim_size);
    if (idx != -1) { gpu_atomic_add(in_diff + idx, out_diff[i]); }
  }
}
...
void BatchGatherKernelUtil<DeviceType::kGPU, T, K>::Forward(DeviceCtx* ctx, const T* in,
  ...
  BatchGatherForwardGpu<T, K>
      <<<BlocksNum4ThreadsNum(elem_cnt), kCudaThreadsNumPerBlock, 0, ctx->cuda_stream()>>>(
          elem_cnt, in, indices, indices_num, instance_size, gather_dim_size, lower_bound, out);
}

template<typename T, typename K>
void BatchGatherKernelUtil<DeviceType::kGPU, T, K>::Backward(
  ...
  BatchGatherBackwardGpu<T, K>
      <<<BlocksNum4ThreadsNum(elem_cnt), kCudaThreadsNumPerBlock, 0, ctx->cuda_stream()>>>(
          elem_cnt, out_diff, indices, indices_num, instance_size, gather_dim_size, lower_bound,
          in_diff);
}
