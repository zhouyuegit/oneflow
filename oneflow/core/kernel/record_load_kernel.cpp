...
void RecordLoadKernel::VirtualKernelInit(const ParallelContext* parallel_ctx) {
  ...
  if (Global<JobDesc>::Get()->IsTrain()) {
    ...
    if(record_load_conf.internal_storage()){
      in_stream_.reset(new PersistentInStream(
        DataFS(), data_paths, false, Global<JobDesc>::Get()->save_downloaded_file_to_local_fs()));
      record_reader_.reset(new PreLoadOFRecordReader(in_stream_.get()));
    } else {
      in_stream_.reset(new PersistentInStream(
