...
__global__ void MeanSquaredLossCopyLabel2DiffGpu(const int64_t elem_cnt,
...
struct MeanSquaredLossKernelUtil<DeviceType::kGPU, PredType, LabelType> {
  static void Forward(DeviceCtx* ctx, const int64_t inst_num,
    ...
    MeanSquaredLossCopyLabel2DiffGpu<<<BlocksNum4ThreadsNum(n),
                                       kCudaThreadsNumPerBlock, 0,
                                       ctx->cuda_stream()>>>(n, label, diff);
    KernelUtil<DeviceType::kGPU, PredType>::Axpy(
