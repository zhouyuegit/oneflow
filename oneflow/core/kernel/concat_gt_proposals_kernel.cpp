...
void ConcatGtProposalsKernel<device_type, T>::ForwardDim0ValidNum(
    const KernelCtx& ctx, std::function<Blob*(const std::string&)> BnInOp2Blob) const {
  // do nothing
}

template<DeviceType device_type, typename T>
void ConcatGtProposalsKernel<device_type, T>::ForwardDim1ValidNum(
    const KernelCtx& ctx, std::function<Blob*(const std::string&)> BnInOp2Blob) const {
  // do nothing
}

template<DeviceType device_type, typename T>
void ConcatGtProposalsKernel<device_type, T>::ForwardRecordIdInDevicePiece(
    const KernelCtx& ctx, std::function<Blob*(const std::string&)> BnInOp2Blob) const {
  // do nothing
}

template<DeviceType device_type, typename T>
void ConcatGtProposalsKernel<device_type, T>::ForwardDataContent(
    const KernelCtx& ctx, std::function<Blob*(const std::string&)> BnInOp2Blob) const {
  ...
  // CHECK_GE(out_proposals_blob->ByteSizeOfBlobHeader(),
  // in_proposals_blob->ByteSizeOfBlobHeader()); Memcpy<DeviceType::kCPU>(ctx.device_ctx,
  // out_proposals_blob->mut_header_ptr(),
  ...
  Blob* out_proposals_blob = BnInOp2Blob("out");
  out_proposals_blob->set_dim0_valid_num(0, 400);
}

ADD_DEFAULT_KERNEL_CREATOR(OperatorConf::kConcatGtProposalsConf, ConcatGtProposalsKernel,
