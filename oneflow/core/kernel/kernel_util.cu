...
static std::vector<std::function<cudaError_t(size_t&, int)>> g_sum_fns = {
    nullptr, nullptr,
    [](size_t& tmp_storage_size, int sum_elem_num) {
      return cub::DeviceReduce::Sum<float*, float*>(nullptr, tmp_storage_size, nullptr, nullptr,
                                                    sum_elem_num);
    },
    [](size_t& tmp_storage_size, int sum_elem_num) {
      return cub::DeviceReduce::Sum<double*, double*>(nullptr, tmp_storage_size, nullptr, nullptr,
                                                      sum_elem_num);
    }};

size_t GetTmpSizeForReduceSum(DataType data_type, int64_t sum_elem_num) {
  size_t tmp_storage_size;
  g_sum_fns[data_type](tmp_storage_size, sum_elem_num);
