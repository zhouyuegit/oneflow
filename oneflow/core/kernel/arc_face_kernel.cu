...
template<typename T, typename K>
struct ArcFaceKernelUtil<DeviceType::kGPU, T, K> final {
...
template<typename T, typename K>
void ArcFaceKernelUtil<DeviceType::kGPU, T, K>::Forward(DeviceCtx* ctx, const int64_t batch_num,
...
template<typename T, typename K>
void ArcFaceKernelUtil<DeviceType::kGPU, T, K>::Backward(DeviceCtx* ctx, const int64_t batch_num,
...
#define MAKE_ARC_FACE_KERNEL_UTIL_ENTRY(in_type_pair, index_type_pair)                \
  template struct ArcFaceKernelUtil<DeviceType::kGPU, OF_PP_PAIR_FIRST(in_type_pair), \
                                    OF_PP_PAIR_FIRST(index_type_pair)>;
OF_PP_SEQ_PRODUCT_FOR_EACH_TUPLE(MAKE_ARC_FACE_KERNEL_UTIL_ENTRY, FLOATING_DATA_TYPE_SEQ,
                                 INT_DATA_TYPE_SEQ);
#undef MAKE_ARC_FACE_KERNEL_UTIL_ENTRY
