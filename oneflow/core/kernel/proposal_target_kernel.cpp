...
void ProposalTargetKernel<device_type, T>::ForwardDim0ValidNum(
    const KernelCtx& ctx, std::function<Blob*(const std::string&)> BnInOp2Blob) const {
  // do nothing
}

template<DeviceType device_type, typename T>
void ProposalTargetKernel<device_type, T>::ForwardRecordIdInDevicePiece(
    const KernelCtx& ctx, std::function<Blob*(const std::string&)> BnInOp2Blob) const {
  // do nothing
}

template<DeviceType device_type, typename T>
void ProposalTargetKernel<device_type, T>::ForwardDataContent(
    const KernelCtx& ctx, std::function<Blob*(const std::string&)> BnInOp2Blob) const {
  ...
  FOR_RANGE(size_t, i, 0, num_out_rois) {
    if (rois_blob->has_record_id_in_device_piece_field()) {
      sampled_rois_blob->set_record_id_in_device_piece(i, 0);
      sampled_roi_inds_blob->set_record_id_in_device_piece(i, 0);
      class_labels_blob->set_record_id_in_device_piece(i, 0);
      regression_targets_blob->set_record_id_in_device_piece(i, 0);
      regression_weights_blob->set_record_id_in_device_piece(i, 0);
    }
  }
}

template<DeviceType device_type, typename T>
void ProposalTargetKernel<device_type, T>::InitializeOutputBlob(
  ...
  Blob* regression_targets_blob = BnInOp2Blob("regression_targets");
  Blob* regression_weights_blob = BnInOp2Blob("regression_weights");
  Memset<DeviceType::kGPU>(ctx, sampled_rois_blob->mut_dptr(), 0,
                           sampled_rois_blob->ByteSizeOfDataContentField());
  Memset<DeviceType::kGPU>(ctx, sampled_roi_inds_blob->mut_dptr(), 0,
                           sampled_roi_inds_blob->ByteSizeOfDataContentField());
  Memset<DeviceType::kGPU>(ctx, class_labels_blob->mut_dptr(), 0,
                           class_labels_blob->ByteSizeOfDataContentField());
  Memset<DeviceType::kGPU>(ctx, regression_targets_blob->mut_dptr(), 0,
                           regression_targets_blob->ByteSizeOfDataContentField());
  Memset<DeviceType::kGPU>(ctx, regression_weights_blob->mut_dptr(), 0,
...
//             best_match_gt_indices_ptr + best_match_gt_indices_blob->static_shape().elem_cnt(),
//             -1);
...
ADD_DEFAULT_KERNEL_CREATOR(OperatorConf::kProposalTargetConf, ProposalTargetKernel,
