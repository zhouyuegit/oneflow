...
class ClientCall final : NonCopyMoveable {
 public:
  ClientCall() = default;
  ~ClientCall() = default;

  static constexpr const size_t I = static_cast<size_t>(kMethod);
  using T = typename std::tuple_element<I, RequestType>::type;
  using U = typename std::tuple_element<I, ResponseType>::type;

  T* mut_request() { return &request_; }
