...
bool CudnnConvCtxCache::InferCudnnConvAlgoCtxWithConfig(
    ...
    CudnnConvAlgoCtx* conv_algo_ctx, const std::string& op_name) const {
  ...
  if (GlobalJobDesc().job_conf().has_cudnn_conv_force_fwd_algo()) {
    ...
    auto fwd_algo_perf = FindCudnnConvAlgorithm<cudnnConvolutionFwdAlgoPerf_t>(args, false);
    ...
    LOG(INFO) << "cudnn conv fwd algo infer: " << op_name
              << ", fwd_algo: " << conv_algo_ctx->fwd_algo
              << ", algo_workspace_size: " << conv_algo_ctx->fwd_ws_size
              << ", max_workspace_size: " << max_buf_size;
  }
  if (GlobalJobDesc().job_conf().has_cudnn_conv_force_bwd_filter_algo()) {
    ...
    auto bwd_filter_algo_perf =
        FindCudnnConvAlgorithm<cudnnConvolutionBwdFilterAlgoPerf_t>(args, false);
  ...
  if (GlobalJobDesc().job_conf().has_cudnn_conv_force_bwd_data_algo()) {
    ...
    auto bwd_data_algo_perf =
        FindCudnnConvAlgorithm<cudnnConvolutionBwdDataAlgoPerf_t>(args, false);
...
bool CudnnConvCtxCache::FindCudnnConvAlgoCtxWithConfig(
    const BlobDesc& in_blob_desc, const BlobDesc& out_blob_desc, const BlobDesc& filter_blob_desc,
    const PbMessage& conf, const size_t max_buf_size, const bool enable_true_half,
    CudnnConvAlgoCtx* conv_algo_ctx, const std::string& op_name) {
    ...
    *conv_algo_ctx = algo_ctx_it->second;
    ...
    bool found =
        InferCudnnConvAlgoCtxWithConfig(in_blob_desc, out_blob_desc, filter_blob_desc, conf,
                                        max_buf_size, enable_true_half, conv_algo_ctx, op_name);
    if (found) {
