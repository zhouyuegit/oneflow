...
void BlobDesc::set_has_instance_available_elem_cnt_field(bool val) {
  CHECK(!header_is_opaque_);
  has_instance_available_elem_cnt_ = val;
}
...
void BlobDesc::InstanceAvailableElemCntFieldToProto(FieldHeaderDesc* proto) const {
  FieldDesc instance_available_elem_cnt_field(Shape({body_field_.shape().At(0)}), DataType::kInt32);
  instance_available_elem_cnt_field.ToProto(proto->mutable_instance_available_elem_cnt());
}
...
void BlobDesc::HeaderToProto(BlobDescProto* proto) const {
  ...
  if (!header_is_opaque_) {
    ...
    if (has_instance_available_elem_cnt_field()) {
      InstanceAvailableElemCntFieldToProto(field_header);
    }
  } else {
